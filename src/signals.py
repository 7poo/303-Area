...
SIGNALS_VERSION = "market-signals-v0.1"
VALID_MATCH_TYPES = ("same_product", "substitute", "near_match")
...
def build_signals(conn: duckdb.DuckDBPyConnection, created_at: datetime) -> None:
    ...
    conn.execute("""
        ...
        WITH source_keys AS (
            ...
            SELECT p.*
            FROM products p
            JOIN source_keys s USING (country_code)
            WHERE p.shop_id = s.source_shop_id AND p.item_id = s.source_item_id
        ), valid_peers AS (
            SELECT country_code, source_shop_id, source_item_id,
                   target_shop_id, target_item_id
            FROM peer_groups
            WHERE peer_status = 'peer_found'
        ), peer_daily AS (
            SELECT v.country_code, v.source_shop_id, v.source_item_id,
                   p.snapshot_date,
                   COUNT(*) FILTER (WHERE p.price IS NOT NULL AND p.price > 0) AS peer_count,
                   MIN(p.price) FILTER (WHERE p.price IS NOT NULL AND p.price > 0) AS peer_min_price,
                   MEDIAN(p.price) FILTER (WHERE p.price IS NOT NULL AND p.price > 0) AS peer_median_price,
                   MAX(p.price) FILTER (WHERE p.price IS NOT NULL AND p.price > 0) AS peer_max_price,
                   MEDIAN(p.discount_percent) FILTER (WHERE p.discount_percent IS NOT NULL) AS peer_median_discount_percent,
                   MEDIAN(p.monthly_sold_value) FILTER (WHERE p.monthly_sold_value IS NOT NULL) AS peer_median_monthly_sold,
                   MEDIAN(COALESCE(p.rating_count, 0) + COALESCE(p.liked_count, 0)) AS peer_median_engagement
            FROM valid_peers v
            ...
            SELECT v.country_code, v.source_shop_id, v.source_item_id,
                   e.snapshot_date,
                   SUM(COALESCE(e.price_down, 0))::INTEGER AS price_down_peer_count,
                   SUM(COALESCE(e.promotion_started, 0))::INTEGER AS promotion_peer_count
            FROM valid_peers v
            ...
            SELECT l.*,
                   ...
                   CASE WHEN prev_source_sold > 0 AND source_monthly_sold IS NOT NULL
                        THEN (source_monthly_sold / prev_source_sold - 1.0) * 100 END AS source_sales_momentum_pct,
                   CASE WHEN prev_peer_sold > 0 AND peer_median_monthly_sold IS NOT NULL
                        THEN (peer_median_monthly_sold / prev_peer_sold - 1.0) * 100 END AS peer_sales_momentum_pct,
            ...
            SELECT c.*,
                   ...
                   + CASE WHEN c.peer_count > 0 THEN LEAST(1.0, c.price_down_peer_count::DOUBLE / c.peer_count) ELSE 0 END * 0.25
                   + CASE WHEN c.peer_count > 0 THEN LEAST(1.0, c.promotion_peer_count::DOUBLE / c.peer_count) ELSE 0 END * 0.20
        ...
        SELECT country_code, snapshot_date, source_shop_id, source_item_id, currency,
               source_price, source_discount_percent, peer_status, peer_count,
               ...
               CASE WHEN peer_count >= 3 THEN 'high' WHEN peer_count >= 1 THEN 'medium' ELSE 'low' END,
               peer_count + price_down_peer_count + promotion_peer_count,
               ?, ?
        FROM scored
...
def build_alerts(conn: duckdb.DuckDBPyConnection, created_at: datetime) -> None:
    ...
    conn.execute("""
        ...
        SELECT a.country_code, a.snapshot_date, a.source_shop_id, a.source_item_id,
               ...
               CASE WHEN a.alert_type IN ('competitor_price_down', 'competitor_promotion_started')
        ...
        FROM (
            ...
            FROM market_signals
            WHERE price_gap_pct >= 10 AND peer_status = 'peer_found'
            UNION ALL
            ...
            FROM market_signals
            WHERE discount_gap_pct <= -10 AND peer_status = 'peer_found'
            UNION ALL
...
def run(db_path: str, min_score: float = 0.45, review_file: str | None = None) -> dict[str, int | float]:
    path = Path(db_path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    parser.add_argument("--min-peer-score", type=float, default=0.45)
