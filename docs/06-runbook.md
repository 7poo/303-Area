...
- `product_matches` top-5 cho mỗi SKU latest snapshot.
- `warehouse/matching_review.csv` gồm 100 cặp chưa gán nhãn.

Chạy local:

```powershell
python -m src.matching --db-path ./warehouse/market.duckdb --backend tfidf --top-k 5 --review-size 100
```
